...
- (CGFloat)alphaForItemWithOffset:(CGFloat)offset
{
    ...
    if (self.type == iCarouselTypeLinear) {
        return (fabs(offset) < 0.0001) ? 1.0 : 0.7;
    }
...
- (CGFloat)valueForOption:(iCarouselOption)option withDefault:(CGFloat)value
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(carousel:valueForOption:withDefault:)]) {
...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeLinear:
        {
            ...
            if (_vertical) {
                baseTransform = CATransform3DTranslate(transform, 0.0, offset * _itemWidth * spacing, 0.0);
            } else {
                baseTransform = CATransform3DTranslate(transform,
                                                        offset * _itemWidth * spacing
            ...
            if (fabs(offset) < 0.0001) {
                return CATransform3DScale(baseTransform, 1.1, 1.1, 1.0);
            }
