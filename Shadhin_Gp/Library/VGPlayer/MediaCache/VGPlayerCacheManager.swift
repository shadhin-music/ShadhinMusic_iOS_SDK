...
import Foundation
 ...
 class VGPlayerCacheManager: NSObject {
    
    static  let VGPlayerCacheConfigurationKey: String = "VGPlayerCacheConfigurationKey"
    static  let VGPlayerCacheErrorKey: String = "VGPlayerCacheErrorKey"
    static  let VGPlayerCleanCacheKey: String = "VGPlayerCleanCacheKey"
    
     static var mediaCacheNotifyInterval = 0.1
    
    fileprivate let ioQueue = DispatchQueue(label: "com.vgplayer.ioQueue")
    fileprivate var fileManager: FileManager!
    
     static let shared = VGPlayerCacheManager()
     private(set) var cacheConfig = VGPlayerCacheConfiguration()
    ...
    static  func cacheConfiguration(forURL url: URL) -> VGPlayerCacheMediaConfiguration {
        let filePath = cacheFilePath(for: url)
        let configuration = VGPlayerCacheMediaConfiguration.configuration(filePath: filePath)
        return configuration
    }
    
     func calculateCacheSize(completion handler: @escaping ((_ size: UInt) -> ())) {
        ioQueue.async {
     ...
     func cleanOldFiles(completion handler: (()->())? = nil) {
        let work: () -> Void = {
            let cacheDirectory = VGPlayerCacheManager.cacheDirectory()
            ...
            if self.cacheConfig.maxCacheSize > 0 && diskCacheSize > self.cacheConfig.maxCacheSize {
                let targetSize = self.cacheConfig.maxCacheSize / 2
