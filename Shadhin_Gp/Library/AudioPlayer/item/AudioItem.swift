...
open class AudioItem: NSObject {
    ...
    func toGpContent() -> GPContent {
        let contentId = Int(contentData?.contentID ?? "")
        let contentType = contentData?.contentType
        let titleBn = contentData?.titleBn ?? contentData?.title ?? ""
        let titleEn = contentData?.titleEn ?? contentData?.title ?? ""
        let imageUrl = contentData?.image ?? ""
        let imageWebUrl = contentData?.newBannerImg ?? imageUrl
        let duration = Int(contentData?.duration ?? "") ?? 0
        let streamingUrl = contentData?.playUrl ?? ""
        let isPaid = contentData?.isPaid ?? false
        let likeCount = contentData?.playCount ?? 0

        let artists = [GPArtist(
            id: Int(contentData?.artistId ?? ""),
            name: contentData?.artist,
            image: contentData?.artistImage
        )]

        let ownership: GPOwnership? = {
            return GPOwnership(
                label: contentData?.ownership?.label,
                copyright: contentData?.ownership?.copyright,
                productBy: contentData?.ownership?.productBy,
                publication: contentData?.ownership?.publication
            )
        }()

        let release: GPRelease? = {
            return GPRelease(id: contentData?.release?.id, name: contentData?.release?.name, date: contentData?.release?.date)
        }()

        let track: GPTrack? = {
            return GPTrack(
                streamingUrl: contentData?.track?.streamingUrl,
                duration: contentData?.track?.duration,
                trackType: contentData?.track?.trackType,
                isLive: contentData?.track?.isLive
            )
        }()

        let genres = [GPGenre(id: contentData?.genres?.id, name: contentData?.genres?.name, image: contentData?.genres?.image)]
        let moods = [GPMood(id: contentData?.moods?.id, name: contentData?.moods?.name, image: contentData?.moods?.image)]
