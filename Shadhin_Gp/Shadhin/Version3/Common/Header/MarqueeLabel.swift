...
//  Shadhin_Gp_Examaple
//
//  Created by MD Murad Hossain on 9/11/25.
//
...
extension MarqueeLabel {
    func marqueeLabel(text: String) {
        self.type = .continuous
        self.speed = .rate(40)
        self.fadeLength = 30.0
        self.text = text
    }
}

extension UILabel {
    func startLabelMarquee(text: String,
                       speed: CGFloat = 40,
                       fadeLength: CGFloat = 10.0,
                       labelSpacing: CGFloat = 20.0) {
        
        self.subviews.forEach { $0.removeFromSuperview() }

        let marquee = MarqueeLabel(frame: self.bounds, duration: 10.0, fadeLength: fadeLength)
        marquee.translatesAutoresizingMaskIntoConstraints = false
        marquee.type = .continuous
        marquee.speed = .rate(speed)
        marquee.fadeLength = fadeLength
        marquee.textAlignment = self.textAlignment
        marquee.textColor = self.textColor
        marquee.font = self.font
        marquee.text = text
