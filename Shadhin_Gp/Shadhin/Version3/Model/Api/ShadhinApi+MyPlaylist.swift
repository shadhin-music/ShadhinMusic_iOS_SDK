...
extension ShadhinApi{
    ...
    func getBothPlaylistsData(completion: @escaping (_ data: [PlaylistsObj.PlaylistDetails]?, Error?) -> Void) {
        ...
        dispatchGroup.notify(queue: .global()) {
            ...
            DispatchQueue.main.async {
                completion(oldData, nil) // Always return data, never treat empty as error
            }
        }
    }
    ...
    func getAllUserPlaylists(
        ...
        var urlRequest = URLRequest(url: URL(string: GET_USER_PLAYLISTS)!)
        urlRequest.cachePolicy = .reloadIgnoringLocalAndRemoteCacheData  // ← Force fresh data
        urlRequest.httpMethod = "GET"
        API_HEADER.forEach { urlRequest.setValue($0.value, forHTTPHeaderField: $0.name) }
    ...
    func getUserPlaylists(
        ...
        var urlRequest = URLRequest(url: URL(string: GET_NEW_USER_PLAYLIST(ShadhinCore.instance.defaults.userIdentity))!)
        urlRequest.cachePolicy = .reloadIgnoringLocalAndRemoteCacheData  // ← Force fresh data
        urlRequest.httpMethod = "GET"
        API_HEADER.forEach { urlRequest.setValue($0.value, forHTTPHeaderField: $0.name) }
